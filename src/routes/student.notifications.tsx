...
export const Route = createFileRoute("/student/notifications")({
  head: () => ({
    meta: [
      { title: "Notifications — Student — ClassSync" },
      { name: "description", content: "Timetable and class updates for students." },
    ],
  }),
...
function StudentNotificationsPage() {
  ...
  const list = notifications.filter((n) => n.role === "student" || n.role === "all");

  return (
    <DashboardShell
      role="student"
      title="Notifications & Class Alerts"
      description="Stay up to date with timetable publications, room changes, and campus notices."
    >
      <Card>
        <CardContent className="p-4">
          {list.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
              ...
              <p className="mt-2 text-sm">No notifications right now.</p>
            </div>
          ) : (
